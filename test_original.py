...
from jukebox.hparams import Hyperparams, setup_hparams, HPARAMS_REGISTRY
...
def test_sampling(model,device, tokens = 40):
    ...
    HPARAMS_REGISTRY[f"prior_{model}"]['min_duration'] = 0 # set the minium duration of the model to 0 to generate only 40 tokens
    vqvae, *priors = MODELS[model]
    vqvae = make_vqvae(
        setup_hparams(vqvae, dict(sample_length=264576)), device
    )  # before : 1048576, 2645888
...
def test_prime_samling(model,device, tokens=40):
    ...
    upsamplers = [
        make_prior(setup_hparams(prior, dict()), vqvae, device) for prior in priors[:-1]
    ]
    labels = [upsamplers[0].labeller.get_batch_labels(metas, device), upsamplers[0].labeller.get_batch_labels(metas, device), None]
    
    set_seed(0)
    hps.sample_length = tokens *  upsamplers[1].raw_to_tokens
    zs = [None,upsamplers[-1].encode(x, start_level=1, bs_chunks=x.shape[0])[0].to(device), torch.cat((zs[-1], torch.zeros(1, 1000000 - zs[-1].shape[-1]).to(device)), dim=-1).long()]
    zs = _sample(zs, labels, sampling_kwargs,  [None,upsamplers[1],None], [1], hps)
    log_zs(zs, 1, f"primed-{model}-{device}")
    
    set_seed(0)   
    hps.sample_length = tokens *  upsamplers[0].raw_to_tokens
    zs = [upsamplers[-1].encode(x, start_level=0, bs_chunks=x.shape[0])[0].to(device),torch.cat((zs[1], torch.zeros(1, 1000000 - zs[1].shape[1]).to(device)), dim=-1).long(),torch.zeros(1, 1000000 ).to(device).long()]
    zs = _sample(zs, labels, sampling_kwargs,   [upsamplers[0],None,None] , [0], hps)
...
test_sampling("5b_lyrics","cuda", tokens = 40)
